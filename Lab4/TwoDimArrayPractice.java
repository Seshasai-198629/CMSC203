   ...
   public int findMinimum( int column )
   {
    ...
    // Write your code here:
	   ...
	   for (int i = 0; i < intArray.length; i++) {
			...
			if (mVal == -1 || val < mVal) { // Using a less-than operator, We want the first match (Eg. 60 then another 60 in the next row)
				mVal = val;
				mRow = i; 
			}
	   }
	   
	   // animate the column & row
	   if (mRow >= 0 && mVal > 0) {
		   //System.out.println("Found value " + mVal + " at r:c " + mRow + ":" + column);
		   animate(mRow, column, mVal);
	   }
    ...
    // Write your code here:
	   ...
	   for (int i = 0; i < intArray.length; i++) {
		   ...
		   for (int x = 0; x < row.length; x++) {
			   ...
			   if (col == value) { 
				   //System.out.println("Found (" + value + ") at r:c " + i + ":" + x);
				   count++;
				   animate(i, x, value);
			   }
		   }
